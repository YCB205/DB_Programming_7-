getManagersIntoManagement();
function getManagersIntoManagement() {
    // 서버로 데이터 요청
    const name = document.querySelector('.input_store_name').value;
    fetch(`/managers?idManager&name=${name}`, {
        method: 'GET',
        headers: {
        ...
        .then(data => {
            ...
            data.Managers.forEach(manager => {
                ...
                // 나머지 데이터를 추가
                for (const key of ['idManager', 'name', 'phoneNumber', 'email']) {
                    const cell = document.createElement('td');
                    cell.textContent = manager[key];
                    row.appendChild(cell);
                }

                // 수정 버튼 추가
                const editCell = document.createElement('td');
                editCell.style.backgroundColor = "#ffffff";
                editCell.style.borderBottomColor = "#ffffff";
                const editButton = document.createElement('a');
                editButton.className = 'btn btn-info';
                editButton.target = '_self';
                // 수정 버튼 클릭 시 showPopupStoreManager 함수 호출
                editButton.onclick = function () {
                    showPopupManager('UpdateManager_information_check.html', 600, 617, manager['idManager']);
                };
